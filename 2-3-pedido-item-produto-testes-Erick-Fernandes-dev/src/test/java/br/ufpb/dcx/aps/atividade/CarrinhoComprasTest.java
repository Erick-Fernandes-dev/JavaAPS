...
class CarrinhoComprasTest {
    ...
    void addProduto() {
        ...
        assertEquals(2,carrinho.getItem("456").getQuantidade(),"Espera-se que haja apenas 1 produto com código '456'");

        carrinho.addProduto(new Produto("456"));
        assertEquals(2,carrinho.getQuantidadeItens());

        System.out.println(carrinho.getQuantidadeItens());

        assertEquals(2,carrinho.getItem("456").getQuantidade(),"Agora espera-se que haja quantidade 2 do produto `456`");
        carrinho.addProduto(new Produto("456"));
        System.out.println(carrinho.getItem("456").getQuantidade());
    ...
    void testQuantidade() {
        ...
        Exception e = assertThrows(CarrinhoComprasException.class, () -> carrinho.setQuantidade("555",2), "Não deve ser possível definir uma quantidade para um produto que não está no carrinho");
